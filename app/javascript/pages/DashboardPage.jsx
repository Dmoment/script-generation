...
const Topbar = ({ user, onLogout }) => (
  <div className="bg-gradient-to-r from-white via-white to-brand-50/30 border-b border-gray-100/60 backdrop-blur-sm">
    <div className="flex items-center justify-between px-8 py-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-900">Good Morning{user?.name ? `, ${user.name.split(' ')[0]}` : ''}</h1>
      ...
      <div className="flex items-center gap-4">
        ...
        <button
          onClick={onLogout}
          className="ml-2 px-3 py-2 text-sm font-medium text-gray-700 hover:text-gray-900 hover:bg-gray-100 rounded-lg transition-colors"
        >
          Logout
        </button>
      </div>
...
const ProjectCard = ({ title, date, status = 'Active' }) => {
  const statusColors = {
            ...
            <div className="mb-8">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                ...
                <div className="rounded-2xl bg-white p-6 shadow-soft-lg border border-gray-100/50">
                  ...
                  <div className="text-2xl font-bold text-gray-900 mb-1">$221.5k <span className="text-lg font-normal text-gray-500">/ $400k</span></div>
                  <div className="text-xs text-gray-500 mb-3">Lorem ipsum is simply dummy text of the printing and typesetting industry.</div>
